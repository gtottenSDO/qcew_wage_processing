...
```{r}
geography_crosswalk <- pull_db_table(schema = "xwalk", table = "area_county")

geography_crosswalk_pm <- pull_db_table(schema = "xwalk", table = "area_pm")
...
qcew_region <- qcew_data |>
  inner_join( # inner because of QCEW data that is not in a county
    geography_crosswalk |> 
  ...
  # calculate regional averages for each industry and year, excluding state totals (county_fips == "000") and regional totals (county_fips == "500")
  summarize(
    region_avg_emp = na_if(sum(avg_emp, na.rm = TRUE), 0),
    region_tot_wage = sum(tot_wage, na.rm = TRUE),
    region_avg_wage = weighted.mean(avgannualwage, avg_emp, na.rm = TRUE),
    .by = c(year, area_id, ind_code)
  ) |> 
    arrange(year, area_id, ind_code) |> 
    mutate(
      region_avg_wage_all_industries = region_avg_wage[ind_code == "00"],
      region_boundary_low = 0.8 * region_avg_wage_all_industries,
      region_boundary_high = 1.2 * region_avg_wage_all_industries,
      wage_cat = factor(
        case_when(
          is.na(region_avg_wage) ~ NA_character_,
          region_avg_wage < region_boundary_low ~ "Low",
          region_avg_wage > region_boundary_high ~ "High",
          region_avg_wage == region_avg_wage_all_industries ~ "Average",
          TRUE ~ "Mid"
        ),
...
avg_wage_postgres <- pull_db_table("econ", "average_wage_region")

#compare to postgres data
qcew_region_comparison <- qcew_region |>
  left_join(
    avg_wage_postgres,
    by = c("year", "area_id" = "region", "ind_code"),
    suffix = c("_new", "_old")
  ) |> 
    mutate(
      avg_emp_diff = abs(region_avg_emp_new - region_avg_emp_old),
      tot_wage_diff = abs(region_tot_wage_new - region_tot_wage_old),
      avg_wage_diff = abs(region_avg_wage_new - region_avg_wage_old),
      max_diff = pmax(avg_emp_diff, avg_wage_diff)
    ) |> 
      filter(ind_code != "99")

  write_csv(qcew_region, "avg_wage_region.csv")
  
  qcew_state <- qcew_data |>
    filter(county_fips == "000") |>
    mutate(
      state_avg_wage = weighted.mean(avgannualwage, avg_emp, na.rm = TRUE),
    .by = c(year, ind_code)
    ) |> 
  select(
    year,
    ind_code,
    state_avg_emp = avg_emp,
    state_tot_wage = tot_wage,
    state_avg_wage
  ) 
  
qcew_wages <- qcew_data |> 
  left_join( # inner because of QCEW data that is not in a county
    geography_crosswalk_pm |> 
      filter(area_type == "region") |>
      inner_join(
        qcew_region, 
        by = "area_id", 
        relationship = "many-to-many"
          ...
          rename(region = area_id), #okay because crosswalk has multiple instances of county for allocating to state and regions as well as own
    by = join_by(year, county_fips, ind_code),
    relationship = "one-to-one"
  ) |>
    left_join(
      qcew_state,
      by = join_by(year, ind_code),
